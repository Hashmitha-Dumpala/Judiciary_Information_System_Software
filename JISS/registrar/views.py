...
from .forms import search_resolved_cases,create_case,allot_hearing_date,status_of_a_case,hearing_data,resolved_data,createjudge,createlawyer
from homepage.models import judge
from homepage.forms import getlawyer
...
class createjudge(View):
    form_class = createjudge
    template_name = 'registrar/create_judge.html'
    ...
    def post(self, request):
        form = self.form_class(request.POST)
        template = 'registrar/judge_created.html'
        if form.is_valid():
            JUDGE = form.save(commit=False)
            JUDGE.save()
           # judge.objects.filter(pk = case.pk).update(cin = case.pk)
          #  context = {
          #      'CIN':case.pk
           # }
...
class createlawyer(View):
    form_class = createlawyer
    template_name = 'registrar/create_lawyer.html'
    ...
    def post(self, request):
        form = self.form_class(request.POST)
        template = 'registrar/lawyer_created.html'
        if form.is_valid():
